...
function getStatusColor(status) {
    ...
    // فقط التحقق من الألوان المحفوظة
...
function removeCustomStatusInternal(status) {
    ...
    // التحقق من أن الحالة غير مستخدمة
...
function renderColorManagement() {
    ...
    // لا توجد حالات أساسية - فقط حالات مخصصة
    const defaultContainer = document.getElementById("colorManagement");
    if (defaultContainer) {
        defaultContainer.innerHTML = `<p style="color: #94a3b8; font-size: 14px;">All statuses are custom. Add your own statuses below.</p>`;
    }
    
    // عرض الحالات المضافة مع إمكانية تعديل الألوان وحذفها
    if (customStatuses.length === 0) {
        container.innerHTML = `<p style="color: #94a3b8; font-size: 14px; text-align: center; padding: 10px;">No custom statuses added yet. Add one using the "Quick Add Custom Status" field above.</p>`;
        return;
    }
